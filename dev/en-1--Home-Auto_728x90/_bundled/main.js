...
function tween(heroFunk) {
	...
	_commonJsCommonJs.tl.from('.t1', t1_time, { y: '+=' + 90, opacity: 0 });
...
function hero_swipe() {
	_commonJsCommonJs.tl.from('.hero', t1_time, { y: '+=' + 90, opacity: 0 }, 't1+=' + t1_time * .8);
}
...
function hero_mask() {
	...
	_commonJsCommonJs.tl.fromTo('.hero', .6, mask_from, mask_to, 't1+=.5');
}
